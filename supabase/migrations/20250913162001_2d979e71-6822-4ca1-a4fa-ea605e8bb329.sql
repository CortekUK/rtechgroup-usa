-- Check and fix existing constraints, then update the function properly

-- First, let's ensure we have the right constraints for payment_applications
-- Drop and recreate if needed to ensure proper naming
DO $$ 
BEGIN
    -- Try to drop existing constraint if it exists with different name
    BEGIN
        ALTER TABLE public.payment_applications DROP CONSTRAINT IF EXISTS ux_payment_app_unique;
    EXCEPTION 
        WHEN OTHERS THEN null;
    END;
    
    -- Add the constraint with consistent naming
    BEGIN
...
-- Ensure pnl_entries reference constraint exists
DO $$ 
BEGIN
    BEGIN
...
BEGIN
  BEGIN
    ...
    -- Insert ledger payment entry - use constraint that definitely exists
