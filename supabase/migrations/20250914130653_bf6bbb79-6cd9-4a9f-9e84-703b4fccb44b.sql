...
-- Migrate existing policy dates to generic start/end dates
UPDATE customer_documents 
SET start_date = policy_start_date, 
    end_date = policy_end_date
WHERE start_date IS NULL AND policy_start_date IS NOT NULL;
...
WHERE NOT EXISTS (
  ...
  WHERE cd.customer_id = insurance_policies.customer_id 
  AND cd.policy_number = insurance_policies.policy_number
