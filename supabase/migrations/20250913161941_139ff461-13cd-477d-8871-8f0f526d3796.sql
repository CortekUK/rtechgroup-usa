-- Check and create only missing constraints

-- Drop the constraint if it exists and recreate it properly
DO $$ 
BEGIN
    -- Check if ux_pnl_entries_reference constraint exists, if not create it
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ux_pnl_entries_reference') THEN
        ALTER TABLE public.pnl_entries 
        ADD CONSTRAINT ux_pnl_entries_reference UNIQUE (reference);
    END IF;
    
    -- Check if ux_payment_applications_unique constraint exists, if not create it  
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'ux_payment_applications_unique') THEN
...
BEGIN
  ...
  BEGIN
    ...
    -- Insert ledger payment entry (idempotent using proper constraint)
    ...
    -- Insert P&L revenue entry (idempotent using proper constraint)
    IF v_has_side_column THEN
      ...
      ON CONFLICT ON CONSTRAINT ux_pnl_entries_reference DO NOTHING;
    ELSE
      INSERT INTO public.pnl_entries (
    ...
    IF p_rental_id IS NOT NULL AND v_category = 'Rental' THEN
      ...
      LOOP
        ...
        -- Create payment application (using proper constraint)
        INSERT INTO payment_applications (payment_id, charge_entry_id, amount_applied)
        VALUES (p_payment_id, v_charge.id, v_to_apply)
        ON CONFLICT ON CONSTRAINT ux_payment_applications_unique DO NOTHING;
