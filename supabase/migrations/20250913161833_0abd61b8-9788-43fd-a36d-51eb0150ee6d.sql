-- Create missing unique constraints for proper payment processing

-- Create unique constraint on ledger_entries.payment_id to prevent duplicate payment entries
ALTER TABLE public.ledger_entries 
ADD CONSTRAINT ux_ledger_entries_payment_id UNIQUE (payment_id);

-- Create unique constraint on pnl_entries.reference to prevent duplicate P&L entries
ALTER TABLE public.pnl_entries 
ADD CONSTRAINT ux_pnl_entries_reference UNIQUE (reference);

-- Create unique constraint on payment_applications to prevent duplicate applications
